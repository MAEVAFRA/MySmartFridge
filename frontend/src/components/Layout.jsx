...
import { Refrigerator, Package, MapPin, ShoppingCart, AlertTriangle, LogOut, Users, ChevronDown, Home, UserCircle, ChefHat, BarChart3, Wallet, ScanLine, Tag, History } from 'lucide-react'
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/profile" className="text-sm text-gray-600 hover:text-primary-600 hidden sm:inline transition-colors">
        ...
        <div className="flex justify-around">
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} className={({ isActive }) =>
              `flex flex-col items-center py-3 px-4 text-xs ${isActive ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-500'}`
            }>
      ...
      <div className="max-w-7xl mx-auto flex">
        <aside className="hidden md:block w-64 bg-white border-r min-h-[calc(100vh-73px)] p-4">
          <nav className="space-y-2">
            {navItems.map(({ to, icon: Icon, label }) => (
              <NavLink key={to} to={to} className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${isActive ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'}`
              }>
